...
package com.hitss.rec.hook.linkedin.action;
...
public class LinkedInSignInAction extends BaseStrutsPortletAction {
	...
	public void processAction(StrutsPortletAction originalStrutsPortletAction, PortletConfig portletConfig, ActionRequest actionRequest, ActionResponse actionResponse)
		...
		if (Validator.isNull(user)) {
			...
			System.out.println(companyNameIn);
			...
			System.out.println(linkedInCreateOrganizationOption);
			String organizationIdIn = ParamUtil.getString(actionRequest, "companyIdIn");
			System.out.println(organizationIdIn);
			Organization organization = null;
			// Create and associate organization or associate existing
			// organization
			...
			if (!linkedInCreateOrganizationOption.equals("neverJoin")) {
				if (Validator.isNotNull(organizationIdIn)) {
					...
					// Create and associate organization (for non-existing
					// organization)
	...
	private void setExistingUserOnSession(PortletRequest portletRequest, User user) {
	...
	private User createUser(ActionRequest actionRequest, long companyId, String linkedinId, User user, Organization organization) {
...
//			user = UserLocalServiceUtil.addUser(creatorUserId, companyId, autoPassword, password1, password2, autoScreenName, screenName, email, facebookId, openId, locale,
...
//			UserLocalServiceUtil.updateEmailAddressVerified(user.getUserId(), true);
			...
			/*
			 * if(pictureUrl != null && !pictureUrl.equals("")){ URL profileURL
			 * = new URL(pictureUrl); InputStream inputStream =
			 * profileURL.openStream();
			 * 
			 * if(inputStream != null){ UnsyncByteArrayOutputStream
			 * unsyncByteArrayOutputStream = new UnsyncByteArrayOutputStream();
			 * StreamUtil.transfer(inputStream, unsyncByteArrayOutputStream, -1,
			 * true); ImageBag imageBag =
			 * ImageToolUtil.read(unsyncByteArrayOutputStream.toByteArray());
			 * RenderedImage renderedImage = imageBag.getRenderedImage();
			 * 
			 * if (renderedImage != null) { renderedImage =
			 * ImageToolUtil.scale(renderedImage, USERS_IMAGE_MAX_HEIGHT,
			 * USERS_IMAGE_MAX_WIDTH); String contentType = imageBag.getType();
			 * UserLocalServiceUtil.updatePortrait(user.getUserId(),
			 * ImageToolUtil.getBytes(renderedImage, contentType)); } } }
			 */
			setUserExpandoValue(user, linkedinId, companyId, User.class.getName(), LINKED_IN_CUSTOM_ID);
		} catch (Exception e) {
			log.error(e);
		}
		return user;
	...
	private String getAutogeneratedEmailAddress(String firstName, String lastName) {
	...
	private void setUserExpandoValue(User user, String customFieldValue, long companyId, String className, String customFieldName) {
		try {

			ExpandoBridge expandoBridge = user.getExpandoBridge();
			setExpandoValue(expandoBridge, customFieldValue, companyId, User.class.getName(), customFieldName);
	...
	private void setOrganizationExpandoValue(Organization organization, String customFieldValue, long companyId, String className, String customFieldName) {
		ExpandoBridge expandoBridge = organization.getExpandoBridge();
		setExpandoValue(expandoBridge, customFieldValue, companyId, Organization.class.getName(), customFieldName);
	}

	private void setExpandoValue(ExpandoBridge expandoBridge, String customFieldValue, long companyId, String className, String customFieldName) {
		
		System.out.println("setExpandoValue:");
		System.out.println(expandoBridge);
		System.out.println(customFieldValue);
		System.out.println(companyId);
		System.out.println(className);
		System.out.println(customFieldName);
		...
		try {
			column = ExpandoColumnLocalServiceUtil.getColumn(companyId, className, ExpandoTableConstants.DEFAULT_TABLE_NAME, customFieldName);
		} catch (Exception ex) {
			log.error(ex.getMessage());
		}
		...
		if (Validator.isNull(column)) {
			try {
//				column = ExpandoColumnLocalServiceUtil.addColumn(table.getTableId(), customFieldName, ExpandoColumnConstants.STRING);
//				UnicodeProperties properties = expandoBridge.getAttributeProperties(customFieldName);
...
//
////				ResourceLocalServiceUtil.updateResources(companyId, ExpandoColumn.class.getName(), ResourceConstants.SCOPE_INDIVIDUAL,
////						String.valueOf(column.getColumnId()), "");
//				
//				Resource resource = ResourceLocalServiceUtil.getResource(companyId, ExpandoColumn.class.getName(), ResourceConstants.SCOPE_INDIVIDUAL,
//						String.valueOf(column.getColumnId()));
//				ResourcePermissionLocalServiceUtil.setResourcePermissions(companyId, ExpandoColumn.class.getName(), resource.getScope(), resource.getPrimKey(),
//						roleUser.getRoleId(), new String[] { ActionKeys.VIEW, ActionKeys.DELETE, ActionKeys.UPDATE });
			} catch (Exception ex) {
				log.error(ex.getMessage());
			}
		}
	...
	private Organization createOrganization(long companyId, String organizationIdIn, String name) {
...
//			organization = OrganizationLocalServiceUtil.addOrganization(defaultUserId, parentOrganizationId, name, type, recursable, regionId, countryId,
//					ListTypeConstants.ORGANIZATION_STATUS_DEFAULT, null, false, serviceContext);
	...
	private User getUserByLinkedinId(String linkedinId, long companyId) {
		System.out.println("getUserByLinkedinId");
		List<ExpandoValue> values;
		try {
			values = ExpandoValueLocalServiceUtil.getColumnValues(companyId, ClassNameLocalServiceUtil.getClassNameId(User.class), ExpandoTableConstants.DEFAULT_TABLE_NAME,
	...
	private Organization getOrganizationByLinkedinCompanyId(String linkedinCompanyId, long companyId) {
		System.out.println("getOrganizationByLinkedinCompanyId");
		List<ExpandoValue> values;
		try {
			values = ExpandoValueLocalServiceUtil.getColumnValues(companyId, ClassNameLocalServiceUtil.getClassNameId(Organization.class),
					ExpandoTableConstants.DEFAULT_TABLE_NAME, LINKED_IN_COMPANY_ID, linkedinCompanyId, -1, -1);

			System.out.println(linkedinCompanyId);
			System.out.println(values);
			if (values != null && !values.isEmpty()) {
	...
	private Organization processExistingOrgByName(long companyId, String companyNameIn, String organizationIdIn) {
	...
	private Organization getOrganizationByLinkedinCompanyName(String companyNameIn, long companyId) {
		try {
